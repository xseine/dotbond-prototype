...
using static Translator.IntegratedQueryRuntime.TsRegexRepository;
using static Translator.IntegratedQueryRuntime.EndpointGenUtilities;
...
public static class EndpointGenLib
{
    ...
    /// </summary>
    public static async Task<(string, string)> GenerateNewEndpoints(string tsDefinitionsFile, Compilation compilation)
    {
        var retryCnt = 0;
        var tsDefinitionsFileContent = await Observable.FromAsync(() => File.ReadAllTextAsync(tsDefinitionsFile)).Delay(_ => Observable.Timer(TimeSpan.FromMilliseconds(retryCnt++ * 100))).Retry(10).FirstAsync();
        ...
        foreach (Match tsDefinition in tsDefinitions) 
            translations.Add(TranslateEndpoint(tsDefinition.Value, compilation));
    ...
    /// </summary>
    private static TranslatedEndpoint TranslateEndpoint(string source, Compilation compilation)
    {
        ...
        (var ctrParams, source, var controllerNamespaces) = ExtractControllers(source, compilation); // Extract controllers
    ...
    private static (List<(string Name, string Type)> controllerInjections, string source, List<string> usingNamespaces)
            ExtractControllers(string source, Compilation compilation)
    {
        ...
        {
            var (actionReturnTypeSymbol, isCollection, isActionResult) = GetActionReturnType(action, controller, syntaxTreesToInspect, compilation);
