...
using Translator.Workspace.IQLogger;
...
public static class EndpointGenInitializer
{
    ...
    public static void InitializeTranslator(FileObservable fileObservable, string csprojPath)
    {
        ...
        var (compilation, languageVersion) = (fileObservable.Compilation, fileObservable.LanguageVersion);
        ...
        Observable.FromEventPattern<FileSystemEventArgs>(watcher, nameof(watcher.Changed)) // Repeat on change
            ...
            .Select(_ => EndpointGenLib.GenerateNewEndpoints(TsDefinitionsFile, compilation).Result) // Generate translated content for queries and controller on top
            .Select(tuple => AfterCompileFixup(ref compilation, languageVersion, tuple.Item1, tuple.Item2)) // After compile, do: spreading, return types and .length
            .AfterComplete((value, cancellationToken) => Task.WhenAll(
                ...
                File.WriteAllTextAsync(Path.Combine(ControllersPath, EndpointSignatureObservable.LockedImplementationsFileName), autogeneratedText + value.Item1, cancellationToken), // Write query implementations to file
...
";
    ...
    private static async Task WriteTsDefinitionsLockFile(CancellationToken cancellationToken)
    {
        ...
        var lockedVersion = Regex.Replace(
                Regex.Replace(tsDefinitionsFileContent, @"@Injectable\(\{.*\}\)", ""),
                @"export class [\s\S]*?(?=\s*@customQuery)", @"export class QueryServiceLock {
