...
import {ClientQuery, IQueryable, Queryable, ServerQuery} from "./queryable";
import {QueryService} from "../query.service";
import {dateFieldsInReturnTypes} from "../return-type-dates";
import {QueryServiceLock} from "../query.service.lock";
import "reflect-metadata";
import {ClientSide, CustomQuery} from "../execution-rules";
// async import
...
export function fromUri(
    ...
    let existingFromUriParameters: number[] =
        Reflect.getOwnMetadata(fromUriMetadataKey, target, propertyKey) || [];
    existingFromUriParameters.push(parameterIndex);
    Reflect.defineMetadata(
        fromUriMetadataKey,
        existingFromUriParameters,
        target,
        propertyKey
    );
}
 ...
 */
export let stackOfCustomQueries: {
    queryName: string;
    args: any;
...
let checkInCache = (
    ...
    requestCacheInCustomQuery[instanceId]?.[baseUrl]?.[queries]?.find(
        (e) =>
            JSON.stringify(e.argumentObject) === JSON.stringify(argumentObject)
    )?.resultPromise;
...
export function customQuery(
    ...
    let accessedProperties = [
        ...method.matchAll(/.find\(\w*\)\s*\??\.\s*(?<property>\w+)/g),
    ].map((e) => (e.groups as any)?.property);
    accessedProperties
        .filter((prop) => !!prop)
        .forEach((prop) => Queryable.accessedPropertiesAfterFind.add(prop));
    ...
    descriptor.value = function () {
        ...
        stackOfCustomQueries.push({
            ...
            args: argumentNames
                .map((argName) => `${propertyName}-${argName}`)
                .reduce(
                    (acc, curr, idx) => ({
                        ...acc,
                        [curr]: argumentValues[idx],
                    }),
                    {}
                ),
            isUnmodified:
                lockedQueryService[propertyName] &&
                lockedQueryService[propertyName]() === method,
        });
        ...
        if (isTopLevelQuery) {
            ...
            isQueryForcedClientSide[queryInstanceId.id] =
                (descriptor.value as any).forceClientSide === true;
        }
 ...
 */
export function implementHttpCallsInController(
    controllerName: string,
    controller: any,
    http: HttpClient,
    serverAddress: string,
    isBaseController: boolean
): void {
    let actions = Object.getOwnPropertyNames(
    ...
    for (let actionName of actions) {
        ...
        controller[actionName] = function () {
            ...
            let body: any;
            if (method === "POST")
                [argumentObject, body] = configurePostRequest(
                    fromUriParameters,
                    fromBodyParameter,
                    argumentObject
                );

            let latestResponseFirstActiveQueryLocal =
                latestResponseFirstActiveQuery;
            ...
            if (isCustomQuery) {
                ...
                if (!cacheHit) {
                    let resultPromise = new Promise(
                        (resolve) => (resultResolve = resolve)
                    );
                    if (!requestCacheInCustomQuery[queryInstanceId.id])
                        requestCacheInCustomQuery[queryInstanceId.id] = {};
                    if (!requestCacheInCustomQuery[queryInstanceId.id][baseUrl])
                        requestCacheInCustomQuery[queryInstanceId.id][baseUrl] =
                            {};
                    if (
                        !requestCacheInCustomQuery[queryInstanceId.id][baseUrl][
                            queryUrls.join(",")
            ...
            let requestObs = cacheHit
                ? (from(cacheHit) as Observable<any>)
                : executeRequest(
                    http,
                    baseUrl,
                    method,
            ...
            return requestObs.pipe(
                tap((e) => {
                    if (queryUrls.length) {
                        latestResponseFirstActiveQueryLocal!.valueOf =
                            function () {
                                return e.firstActiveQuery ?? "";
                            };

                        if (!e.firstActiveQuery) stackOfCustomQueries = [];
                    }

                    if (!cacheHit && isCustomQuery) resultResolve(e);
                })
            );
        };
    }
 ...
 */
function executeRequest<THeaders extends { [p: string]: string | string[] }>(
    http: HttpClient,
    url: string,
    method: "GET" | "POST",
    parameters?: any,
    ...
    if (headers?.["bond-queries"]) {
        ...
        bondTypeDates = bondUrls
            .map((url) => url.split("/").slice(1))
            .flatMap(
                ([controller, action]) =>
                    dateFieldsInReturnTypes[controller]?.[action]
            );
    }
        ...
        map((response) =>
            baseTypeDates
                ? {
                    ...
                    body: Array.isArray(response.body)
                        ? response.body.map((e) =>
                            convertDateFields(e, baseTypeDates)
                        )
                        : convertDateFields(response.body, baseTypeDates),
                }
                : response
        )
    );
...
export function asQueryable<T extends Observable<any>>(input: T): T extends ServerQuery & Observable<infer U> ? IQueryable<U, CustomQuery> : T extends ClientQuery & Observable<infer U> ? IQueryable<U, ClientSide> : Observable<T> {
        ...
        input.pipe(
            map((result) => {
                let currentQueryName =
                    stackOfCustomQueriesLocal.pop()?.queryName;
                let hasActiveEndpoint =
                    latestResponseFirstActiveQueryLocal != "";
                ...
                result =
                    (result as any).firstActiveQuery !== undefined &&
                    (result as any).body
                        ? (result as any).body
                        : result;
                ...
                return {
                    result,
                    shouldUseClientSideProcessing: !hasActiveEndpoint,
                };
            })
        )
    ) as any;
 ...
 *
 */
export function evaluateInnerObservables(
    object: any,
    operatorObservables: Observable<any>[]
): Observable<any> {
    if (typeof object === "string") {
        let matches = [
            ...object.matchAll(
                new RegExp(EMBEDDED_OBSERVABLE_STRING_PLACEHOLDER + "\\d+", "g")
            ),
        ];
        ...
        return combineLatest(
            ...matches
                .map((match, idx) => ({
                    callback: (currentValue: string, observedValue: string) =>
                        object.replace(match[0], observedValue),
            ...
            map((callbacks) =>
                callbacks.reduce(
                    (acc, curr) => curr.callback(acc, curr.value),
                    object as string
                )
            )
        );
    ...
    let findObservablesInChildren = (parent: any) => {
        ...
        let childrenObservables: {
            callback: (observedValue: any) => void;
        ...
        for (let [key, value] of Object.entries(parent)) {
            if ((value as any)?.constructor.name === "Observable")
                childrenObservables.push({
                    callback: (observedValue) => (parent[key] = observedValue),
                    observable: value as any,
                });
            else if ((value as any)?.constructor.name === "Object")
                childrenObservables = [
                    ...childrenObservables,
                    ...findObservablesInChildren(value),
                ];
        }
    ...
    let findEmbeddedObservables = (
        parent: any
    ): {
        callback: (observedValue: string) => void;
        observable: Observable<any>;
    }[] => {
        let childrenObservables: {
            callback: (observedValue: string) => void;
        ...
        for (let [key, value] of Object.entries(parent)) {
            if (typeof value === "string") {
                let matches = [
                    ...value.matchAll(
                        new RegExp(
                            EMBEDDED_OBSERVABLE_STRING_PLACEHOLDER + "\\d+",
                            "g"
                        )
                    ),
                ].map((e) => e[0]);
    ...
    if (
        entriesWithObservables.length === 0 &&
        entriesWithEmbeddedObservables.length === 0
    )
        return of(object);

    let observeForInnerObservables = (observable: Observable<any>) =>
        observable.pipe(
            switchMap((value) => {
                if (typeof value !== "object") return of(value);

                let entriesWithObservables = findObservablesInChildren(value);
                let entriesWithEmbeddedObservables =
                    findEmbeddedObservables(value);
                return entriesWithObservables.length === 0 &&
                entriesWithEmbeddedObservables.length === 0
                    ? of(value)
                    : combineLatest(
                        ...entriesWithObservables
                            .concat(entriesWithEmbeddedObservables)
                            .map((e) =>
                                observeForInnerObservables(
                                    e.observable
                                ).pipe(tap((value) => e.callback(value)))
                            )
                    ).pipe(map((_) => value));
            })
        );
    ...
    return combineLatest(
        ...entriesWithObservables
            .concat(entriesWithEmbeddedObservables)
            .map(({callback, observable}) =>
                observeForInnerObservables(observable).pipe(
...
> = Key extends string
    ? T[Key] extends PrimitiveType
        ? Key // FormControl nodes represent custom controls, whose TValue properties cannot be accessed from ControlTree
        : T[Key] extends (infer U)[]
            ? U extends PrimitiveType
                ? `${Key}[%d]`
                : TDepth extends 5
                    ? never
                    : `${Key}[%d].${PathImpl<U, keyof U, Increment<TDepth>>}` // If not a primitive type, use a template string, where arrays have [$d] attached to the end.
            : T[Key] extends object
                ? TDepth extends 5
                    ...
                    : `${Key}.${PathImpl<
                        ...
                        Increment<TDepth>
                    >}`
                : never // Parent.Child.<etc>
    : never;
/**
 * Type safe property access of typescript objects.
 */
export type PrimitiveKey<T> = 0 extends 1 & T
    ? string
    : T extends PrimitiveType
        ? never // Any or primitive
        : T extends (infer U)[]
            ? U extends PrimitiveType
                ? "[%d]"
                : `[%d].${PathImpl<U, keyof U, 0>}` // Array
...
function configurePostRequest(
    ...
    if (fromBodyParameter) {
        ...
        let complexNonUriEntry = Object.entries(argumentObject).find(
            ([k, v]) => !fromUriParameters.includes(k) && typeof v === "object"
        );
        if (complexNonUriEntry[1]) body = JSON.stringify(complexNonUriEntry[1]);
    }

    argumentObject = Object.fromEntries(
        Object.entries(argumentObject).filter(
            ([k, v]) => typeof v !== "object" || fromUriParameters.includes(k)
        )
    );
...
function convertDateFields(object: any, dateFields: string[]) {
    for (let key in object)
        if (dateFields.includes(key)) object[key] = new Date(object[key]);
...
export type ApiControllers = {
    ...
    Extended: {
        [TAction in keyof Omit<
            QueryService,
            keyof BaseEndpointsService
        >]: QueryService[TAction];
    };
};
